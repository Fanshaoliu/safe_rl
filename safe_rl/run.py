import safe_rl
from safe_rl.user_config import DEFAULT_BACKEND
...
MPI_COMPATIBLE_ALGOS = ['cppo', 'csc']
BASE_ALGO_NAMES = ['cppo', 'csc']
...
def parse_and_execute_grid_search(cmd, args):
    ...
    if any([arg in valid_help for arg in args]):
        print('\n\nShowing docstring for safe_rl.'+cmd+':\n')
        print(algo.__doc__)
        sys.exit()
...
if __name__ == '__main__':
    ...
    if cmd in valid_help:
        ...
        special_info = dedent("""
            ...
            \tpython -m safe_rl.run ppo --env HalfCheetah-v2 --clip_ratio 0.1

            If you need a quick refresher on valid kwargs, get the docstring
            with

            \tpython -m safe_rl.run [algo] --help
